<script lang="ts">
    ...
    export let layout: "fitDataTable" | "fitData" | "fitColumns" | "fitDataFill" | "fitDataStretch" = 'fitDataTable';
    ...
    $: if (docente && asignatura) {
        console.log('Reactive loadNotas triggered:', { docente, asignatura });
        ...
        return {
            ...
            cellEdited: async (e: any) => {
                try {
                    ...
                    const noteFields = ['N1','N2','N3','N4','N5','N6','N7','N8','N9','N10','N11','N12'];
                    const noteValues = noteFields.map(f => dataRR[f] || " ");
                    const counts = countNonEmptyGroups(noteValues);

                    noteFields.forEach((field, i) => {
                        const val = dataRR[field];
                        if (val !== " " && val !== "" && val != null) {
                            const num = parseFloat(val);
                            if (!isNaN(num)) {
                                const idx = (i + 1).toString();
                                if (['1','2','3'].includes(idx)) Saber35 += num;
                                else if (['4','5','6'].includes(idx)) Hacer35 += num;
                                else if (['7','8','9'].includes(idx)) Ser20 += num;
                                else if (idx === '10') Autoe5 = num;
                                else if (idx === '11') Coev5 = num;

                                // Set fecha if missing
                                const fechaKey = `fecha${idx}`;
                                if (!dataRR[fechaKey]) dataRR[fechaKey] = toDateInputValue(new Date());
                            }
                        }
                    });
   ...
   const calcTotal = async (cant: number) => {
    const rows = tableInstance.getRows(); // ← Esto devuelve RowComponent[]
    for (let i = 0; i < Math.min(cant, rows.length); i++) {
        try {
            const row = rows[i];
            const data = { ...row.getData() }; // Trabaja con una copia
            ...
            data.Val = finalVal.toFixed(2);

            // ✅ Ahora sí: row es RowComponent y tiene .update()
            row.update(data);

            // Opcional: actualiza datos internos si los usas fuera de Tabulator
            datosTabla[i] = data;
        } catch (error) {
    ...
    const getCurrentPeriodo = async () => {
        try {
            ...
            return periodos.find((p: any) => p.selected === 'selected')?.nombre || 'DefaultPeriod';
        } catch (error) {
    ...
    const Nasignatura = (nivel: string, numero: string, asignatura: string, periodo: string) => {
        consolelog(`Nasignatura: ${nivel}, ${numero}, ${asignatura}, ${periodo}`);
    };
    ...
    const loadNotas = async (targetTableId: string, currentDocente: string, currentAsignatura: string) => {
        ...
        tableInstance = new Tabulator(targetTableId, {
                    ...
                    columns: [
                        {
                            ...
                            cellClick: (_e, cell) => Inasistencias(cell.getRow().getData()),
                        },
                        {
                            title: "Val",
                            field: "Val",
            ...
            ajaxResponse: (_url, _params, response: any[]) => {
                return response.sort((a, b) => a.Nombres.localeCompare(b.Nombres));
            },
            ajaxParams: {
    ...
    const headerMenu = [
        {
            ...
            action: function (_e: Event, column: any) {
                ...
                const porcentajeData = datosTabla.find(d => d[`porcentaje${idx}`]);
                const fechaData = datosTabla.find(d => d[`fechaa${idx}`]);

                aspx = aspectoData ? aspectoData[`aspecto${idx}`] : '';
                poraspx = porcentajeData && !isNaN(Number(porcentajeData[`porcentaje${idx}`]))
                    ? Number(porcentajeData[`porcentaje${idx}`])
                    : '';
                faspx = fechaData?.[`fechaa${idx}`] || toDateInputValue(new Date());
    ...
    const saveAspectos = () => {
        ...
        (window as any).Swal?.fire?.('Éxito', 'Aspectos actualizados (simulado)', 'success');
