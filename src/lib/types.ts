...
export interface AreaOrderPayload {
  year: string;
  nivel: string;
}
...
export interface ValoracionAreasResponseItem {
  estudiante: number;
  nombres: string;
  area: string;
  asignat: string;
...
export interface ConvivenciaDetallado {
  ...
  cv_positivos: string;
  cv_firma: string; // Base64 image
  firmaAcudiente: string; // Base64 image
  cv_fechahora: string;
}

export interface ConvivenciaRecord {
  ind: string;
  estudiante: string;
  nombres: string;
  grupo: string;
...
export interface ConsolidadoConvivenciaPayload {
  estudiante: string;
  year: string;
}
